(function(){
  ...
  const API = {
    initUI(container) {
      ...
      const secStyle = document.createElement("details"); secStyle.className = "qp-main-acc";
      secStyle.style.cssText = "margin-bottom:10px; border:1px solid #ccc; border-radius:6px; background:#fff;";
      const sumStyle = document.createElement("summary"); sumStyle.innerHTML = `<span style="margin-right:8px;">▶</span>🎨 スタイル・文化 (Styles)`;
      sumStyle.style.cssText = "font-weight:bold; padding:10px 14px; cursor:pointer; background:#eef2f6; list-style:none;";
      secStyle.appendChild(sumStyle);
      const conStyle = document.createElement("div"); conStyle.className = "qp-section-content"; conStyle.style.padding = "10px";
      secStyle.appendChild(conStyle);
      Object.entries(STYLES_DATA).forEach(([k,v]) => { conStyle.appendChild(createSubAccordion(k, v)); });
      root.appendChild(secStyle);

      const secEras = document.createElement("details"); secEras.className = "qp-main-acc";
      secEras.style.cssText = "margin-bottom:10px; border:1px solid #ccc; border-radius:6px; background:#fff;";
      const sumEras = document.createElement("summary"); sumEras.innerHTML = `<span style="margin-right:8px;">▶</span>🕰️ 時代・年代 (Eras)`;
      sumEras.style.cssText = "font-weight:bold; padding:10px 14px; cursor:pointer; background:#eef2f6; list-style:none;";
      secEras.appendChild(sumEras);
      const conEras = document.createElement("div"); conEras.className = "qp-section-content"; conEras.style.padding = "10px";
      secEras.appendChild(conEras);
      Object.entries(ERAS_DATA).forEach(([k,v]) => { conEras.appendChild(createSubAccordion(k, v)); });
